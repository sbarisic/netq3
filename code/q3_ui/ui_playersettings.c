...
void UI_GetPlayerHead(qhandle_t* HeadModel, qhandle_t* HeadSkin) {
	...
	headModelSlash = strchr(headModel, '/');
	if (headModelSlash) {
		strcpy(headSkin, headModelSlash + 1);
		*headModelSlash = 0;
	} else
		strcpy(headSkin, "default");

	Com_sprintf(headPath, sizeof(headPath), "models/players/%s/head.md3", headModel);
	*HeadModel = trap_R_RegisterModel(headPath);

	Com_sprintf(headPath, sizeof(headPath), "models/players/%s/head_%s.skin", headModel, headSkin);
	*HeadSkin = trap_R_RegisterSkin(headPath);
}
...
static void PlayerSettings_DrawPlayer(void *self) {
	...
	if (strcmp(buf, s_playersettings.playerModel) != 0) {
		...
		viewangles[PITCH] = 0;
		viewangles[ROLL] = 0;
		UI_PlayerInfo_SetInfo(&s_playersettings.playerinfo, LEGS_WALK, TORSO_STAND, viewangles, vec3_origin,
			((rand() % (WP_NUM_WEAPONS - 2)) + 1), qfalse);
	}
